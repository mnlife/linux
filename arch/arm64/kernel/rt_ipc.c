...
/*
 * arm64 implementation of the rt_ipc partial context switch.
 *
 ...
 *     the subset we overwrite (PC/SP and the TLS base) so it can be put back
 *     precisely, even if the call is preempted, migrated to another CPU,
 ...
 *     all maintained -- never a bare TTBR0_EL1 write.
 *
			     ...
			     const struct rt_ipc_arch_ctx *ctx)
{
	...
	regs->pc = uregs->pc;
	regs->sp = uregs->sp;
	regs->tpidr = read_sysreg(tpidr_el0);
	regs->saved = true;
	 ...
	 * without touching TTBR0_EL1 so userspace never sees a half-migrated
	 * thread.
	 */
	(void)ctx;
	return -EOPNOTSUPP;
...
void rt_ipc_arch_return(struct rt_ipc_frame *frame)
{
	...
	if (regs->saved) {
		...
		uregs->pc = regs->pc;
		uregs->sp = regs->sp;
		write_sysreg(regs->tpidr, tpidr_el0);
	}
